...
void Game(CSquareVector &squares);
...
{
	{
		const string FILE_SQUARES = "monopoly.txt"; // Used for storing the file that stores the square data

		CSquareVector pSquares;

		ReadSquaresFile(FILE_SQUARES, pSquares); // Reads the file

		Game(pSquares); // Runs the game
	}
	_CrtDumpMemoryLeaks();
}

void ReadSquaresFile(string fileName, CSquareVector &squares) // Reads file and puts them into a square array
{
	{
...
void Game(CSquareVector& squares) // Function that runs the game and takes in the array of squares
{
	{
		...
		for (int i = 1; i <= 20; ++i) // Runs the game for 20 rounds
		{
			...
			Round(pPlayerOne, pPlayerTwo, squares);
			Round(pPlayerTwo, pPlayerOne, squares);
		}
...
void Round(shared_ptr<CPlayer> Player, shared_ptr<CPlayer> OtherPlayer, CSquareVector& squares) // Function that runs the rounds code (keeps everthing neat) takes in the player and the array of squares
{
	{
		...
		if (squares[Player->GetPosition()]->GetSquareType() == 1 || squares[Player->GetPosition()]->GetSquareType() == 3)
		{
			...
			if (squares[Player->GetPosition()]->GetIsOwned() && squares[Player->GetPosition()]->GetOwningPlayer() != Player)
			{
				...
				{
					cout << Player->GetName() << " pays " << POUND << "10 for ticket" << endl;
				}
				else
				{
					cout << Player->GetName() << " pays " << POUND << squares[Player->GetPosition()]->GetSquareRent() << endl; // Output
				}
			}
		}
		...
		{
			cout << " " << Player->GetName() << " passes GO and collects " << POUND << "200" << endl << endl; // Outputs the player has passed go and collected 200
			Player->SetMoney(Player->GetMoney() + 200); // Adds £200 to the players money
		}

		cout << Player->GetName() << " has " << POUND << Player->GetMoney() << endl << endl;
...
{
	{
		...
		cout << PlayerOne->GetName() << " has " << POUND << PlayerOne->GetMoney() << endl;
		cout << PlayerTwo->GetName() << " has " << POUND << PlayerTwo->GetMoney() << endl;
